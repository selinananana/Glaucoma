...
class ResBlock_do_fft_bench_eval(nn.Module):
    ...
    def forward(self, x):
        ...
        dim = 1
        y = torch.fft.rfft2(x, norm=self.norm)
        # y = torch.rfft(x, signal_ndim=2, normalized=self.norm)
        # y = torch.rfft(x, signal_ndim=2, onesided=False)
        # y = torch.fft.fft(torch.fft.rfft(x, dim=1, norm=self.norm), dim=0)
        y_imag = y.imag
        y_real = y.real
        y_f = torch.cat([y_real, y_imag], dim=dim)
        y = self.main_fft(y_f)
